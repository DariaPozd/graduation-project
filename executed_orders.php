...
?>
	...
	//закрытие заказа
	if (isset($_GET['close'])) {
		$query = "UPDATE orders SET orders.status = 'выполнен' WHERE order_id = " . $_GET['oId'];
		$statement = $link->prepare($query);
		$statement->execute();
	}
	//обновление количества услуг
	if (isset($_GET['updateQuantity'])) {
		...
		foreach($serv as $value){
			if(isset($_GET[$value['service_id']])){
				$sum = $value['service_price'] * $_GET[$value['service_id']];
				$query = "UPDATE services_in_order SET services_in_order.quantity = " . $_GET[$value['service_id']] . ", services_in_order.sum = " . $sum . "
				WHERE order_id = " . $_GET['oId'] . " AND service_id = " . $value['service_id'];
				$statement = $link->prepare($query);
				$statement->execute();
			}
		}
	...
	$query = "SELECT * FROM orders WHERE status = 'подтвержден'";
	$statement = $link->prepare($query);
	$statement->execute();
	...
	$i = 0;
	if($total_row > 0) {
		echo "<h5 class='order-title'>Количество выполняемых заказов: " . $total_row . "</h5>";
		//добавление новой услуги в заказ
		if (isset($_GET['addServ'])) {
			$values = $_GET['select'];
			foreach ($values as $a){
				$query = "SELECT service_price FROM services WHERE service_id = " . $a;
    			$statement = $link->prepare($query);
				$statement->execute();
				$serv = $statement->fetch();
			    $query = "INSERT INTO services_in_order(order_id, service_id, quantity, sum) VALUES (" . $_GET['oId'] . ", " . $a . ", 1," . $serv[service_price] . ")";
			    $statement = $link->prepare($query);
				$statement->execute();
			}
		}
			...
			<div id="collapse-group">
			    <div class="card" style="margin-top: 30px;">
			 		<div class="card-header" style="background-color: #005b96;">
			    		...
			    		<span style="float: right; color: #fff;"><?php echo date("H:i d.m.Y", strtotime($order[order_date]));?></span>
			    	</div>
				    <div id="<? echo 'el' . $i ?>" class="collapse show" style="background-color: #f7fafb;">
			    		...
			    		?>		    		
				    		<div>
				    			...
				    			<p>Выполняет: <?php echo $empl[s_surname]. " " . mb_substr($empl[s_name], 0, 1, 'UTF-8') . ". " . mb_substr($empl[s_otch], 0, 1, 'UTF-8') . "."; ?></p>
				    		</div>
			            ...
			            ?>
			                    <tr>
			                        ...
			                        <td align="center" class="quantity">
			                        	<input type="number" min="1" name="<?php echo $service_in[service_id];?>" class="edit" value="<?php echo $service_in["quantity"]; ?>">
			                        </td>
			                        <td align="right">
			                            <?php echo number_format($service_in["sum"], 2, '.', ' '); ?>
			                        </td>
			                        <td align="center" id="el">
			                        	<a href="<? echo 'executed_orders.php?sId=' . $service_in[service_id] . '&acd=1&id=' . $order[order_id];?>"><i class="fas fa-trash text-danger"></i></a>
			                        </td>
			                    </tr>
			                <?php
			            	...
			            	<input type="submit" class="btn btn-outline-success"  name="updateQuantity" value="Обновить количество"></input>
			            </form>
				            <form action="<?$_SERVER['PHP_SELF']?>" method="GET">
				            	...
				            	<?php
			            			...
			            			foreach ($service as $s) {
										$out .= "<option value='" . $s[service_id] . "'>" . $s[service_name] . " - " . $s[service_price] . "</option>";
									}
									$out = "<div class='row justify-content-around add-service' style='margin-bottom: 15px;'>
												<div class='col-md-12'><select class='custom-select' multiple='multiple' size='6' name='select[]'>$out</select>
												</div>
											</div>";
									echo $out;
								?>
									<div>
									<input type="submit" class="btn btn-success" name="addServ" value="Добавить услугу"></input>
								</form> 
				            	...
				            	<form action="<?$_SERVER['PHP_SELF']?>" method="GET">
				            		...
				            		<input type="submit" class="btn btn-danger" name="close" value="Заказ выполнен"></input>
				            	</div>
			            	</form>  	
					    </div>
				    </div>
    ...
    <?php   
		...
		echo "<div class='col-md-4 no-orders'>Выполняемых заказов нет</div>";
	}?>
	<script type="text/javascript">
	$('#sendQ').on('click', function() {
	   $('#formQ').submit();
	}) 
	</script><?php
